...
//			System.out.println("Test predicted: "+Arrays.toString(res) + " -> true: "+ Arrays.toString(c.out));
	...
	public static MultiLayerPerceptron LoadData(double epochs) {
		
		HashMap<Integer, Coup> coups = loadGames("./resources/dataset/Tic_tac_initial_results.csv");
		
//		 You can watch on array 
//		Set<Entry<Integer,Coup>> entries = coups.entrySet();
//		for (Map.Entry<Integer,Coup> entry : entries) {
...
//		double epochs = 1000 ;
		...
		return learn(9, mapTrain, config.hiddenLayerSize, config.learningRate, config.numberOfhiddenLayers, true, epochs);
	}
